...
- [x] Stock Accuracy Score (SAS) compares last stock take vs current `stock_pieces`. Used as the marketing metric in CLAUDE.md. Mobile now shows an inventory accuracy metric from immutable `stock_take_counts`; Supabase has the tenant-scoped table scaffold.
...
- W0.7 Reporting & Exports — ✅ daily/weekly/monthly report (incl. hourly pattern), CSV/PDF export, audit log, sync health. Stock Accuracy Score has a mobile/local schema and metric; web reporting rollup remains W0.8/W0.9 work.
...
- [/] Stock Accuracy Score. Local mobile metric and tenant-scoped schema exist; web rollup and marketing proof remain post-scaffold work.
...
- [x] Customer-erasure UI update 2026-05-12: web `/modules` now lists owner/manager customer privacy rows, masks phone values to suffixes, and provides an erasure action that calls `erase_customer_pii(uuid, text)` before revalidating `/modules` and `/audit`.
