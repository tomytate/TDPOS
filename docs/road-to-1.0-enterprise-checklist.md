...
- [x] Local DB: `db/init.ts`, `db/schema.ts` (LOCAL_SCHEMA_SQL), `db/migrations/001_initial_schema.sql` (products, categories, sales, sale_items, sync_queue, receipt_sequence, customers, inventory_logs, settings, schema_version with PRAGMA WAL + foreign_keys ON, idempotent CREATE IF NOT EXISTS), and `runLocalMigrations()` v2 for local Tier B `shift_sessions`.
...
- [/] 2. Fill Tier B Pro mobile shells: tablet POS, owner lanes, shift login, shift handoff. Product-specific surface contracts and native preview panels exist; shift login/handoff now have local SQLite start/close controls; tablet POS and owner lanes still need production controls.
